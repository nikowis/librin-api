...
public interface ConversationRepository extends JpaRepository<Conversation, Long>, JpaSpecificationExecutor<Conversation> {
    ...
    @Query("SELECT c FROM Conversation c LEFT JOIN c.messages m LEFT JOIN FETCH c.offer o LEFT JOIN FETCH o.owner LEFT JOIN FETCH o.buyer JOIN c.customer cust WHERE o.id = :offerId AND (cust.id = :userId OR o.ownerId = :userId)")
